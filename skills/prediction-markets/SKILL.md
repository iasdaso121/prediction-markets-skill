...
description: Read-only market data for Kalshi and Polymarket prediction markets — market discovery, orderbooks, price history, implied probability, and cross-venue price comparison. Use this skill whenever the user mentions Kalshi, Polymarket, prediction markets, event contracts, implied probability, betting odds, "what are the odds of X", or wants prices/probabilities for election, Fed/rates, economic, weather, crypto, or sports OUTCOME markets — even if they never name a venue or say "prediction market". Also use it for comparing the same event's price across venues, spotting price divergence, or pulling probability time series. The bundled scripts prevent hallucinated API endpoints: never write Kalshi/Polymarket API calls from memory when this skill is available. Python stdlib only, no API keys, no trading.
---
